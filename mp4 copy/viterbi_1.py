...
def train_v1(train):
    ...
    for tag in trans_count:
        ...
        # dont smoothing if full transition
        if v == 16:
            for next_tag in cur_dict:
                trans_prob[tag][next_tag] = (cur_dict[next_tag] + t_laplace) / (cur_total + t_laplace * (v + 1))
                trans_prob[tag]["UNKNOWN"] = t_laplace / (cur_total + t_laplace * (v + 1))
        # use smoothing for empty transitions
        else:
            for next_tag in cur_dict:
                trans_prob[tag][next_tag] = (cur_dict[next_tag] + t_laplace) / (cur_total + t_laplace * (v + 1))
                trans_prob[tag]["UNKNOWN"] = t_laplace / (cur_total + t_laplace * (v + 1))
    ...
    for tag in emis_count:
        ...
        for word in cur_dict:
            emis_prob[tag][word] = (cur_dict[word] + e_laplace) / (cur_total + e_laplace * (v + 1))
            emis_prob[tag]["UNKNOWN"] = e_laplace / (cur_total + e_laplace * (v + 1))
