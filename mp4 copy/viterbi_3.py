...
def train_v1(train):
    ...
    for tag in trans_count:
        ...
        if tag in hapx_count_tag:
            cur_laplace = t_laplace * pow(hapx_count_tag[tag], 2)
        else:
...
def best_list(sentence, tag_list, trans_prob, emis_prob, emis_prob_unseen):
    suffix = ["tion", "ity", "er", "ness", "ism", "ment", "ant", "ship", "age",
                "ery", "ise", "en", "ate", "fy", "ify", "less", "ous", "ful", "able"]
    preffix = ["re", "dis", "over", "un", "mis", "out", "co", "sub", "un", "in", "non"]
    to_return = []
    node_list = []
    start_node = Node(0)
        ...
        if i == 1:
            for tag_b in tag_list:
                if cur_word in emis_prob[tag_b]:
                    ...
                    # search for preffix or suffix match
                    for each_pre in preffix:
                        if cur_word.startswith(each_pre):
                            flag = "pre"
                            modification = each_pre
                    for each_suf in suffix:
                        if cur_word.endswith(each_suf):
                            flag = "suf"
                            modification = each_suf

                    # modify word and use unseen_emission
                    if flag == "pre":
                        cur_word = modification + "-X"

                        if tag_b in trans_prob["START"]:
                            if cur_word in emis_prob_unseen[tag_b]:
                                cur_value = math.log(node_list[i - 1].get_tag_prob("START")) + math.log(
                                    trans_prob["START"][tag_b]) + math.log(
                                    emis_prob_unseen[tag_b][cur_word])
                            else:
                                cur_value = math.log(node_list[i - 1].get_tag_prob("START")) + math.log(
                                    trans_prob["START"][tag_b]) + math.log(
                                    emis_prob_unseen[tag_b]["UNKNOWN"])
                        else:
                            if cur_word in emis_prob_unseen[tag_b]:
                                cur_value = math.log(node_list[i - 1].get_tag_prob("START")) + math.log(
                                    trans_prob["START"]["UNKNOWN"]) + math.log(
...
def viterbi_3(train, test):
    ...
    new_train = refine_train(train)
    new_test = refine_test(test)
    ...
    new_pair = train_v1(new_train)
    emis_prob_unseen = new_pair[1]
    ...
    for j in range(len(test)):
        sentence = test[j]
        new_sentence = new_test[j]
        count += 1
        cur_list = best_list(sentence, tag_list, trans_prob, emis_prob, emis_prob_unseen)
        cur_result = []
...
def refine_train(train):
    ...
    for sentence in train:
        ...
        for pair in sentence:
            ...
            for each_preffix in preffix:
                if word.startswith(each_preffix):
                    new_pair = (each_preffix + "-X", tag)
                    break
                else:
                    new_pair = (word, tag)
            for each_suffix in suffix:
                if word.endswith(each_suffix):
                    new_pair = ("X-" + each_suffix, tag)
                    break
...
def refine_test(test):
    suffix = ["ness", "ity", "cy", "ise", "ate", "fy", "tion", "sion", "er", "ers",
                "ment", "ant", "age", "ence", "ery", "ent", "ive", "ous", "ful", "less", "able"]
    preffix = ["re", "dis", "over", "un", "mis", "out", "co", "sub", "un", "in", "non"]
    ...
    for sentence in test:
        ...
        for word in sentence:
            ...
            for each_preffix in preffix:
                if word.startswith(each_preffix):
                    new_word = each_suffix + "-X"
                    break
                else:
                    new_word = word
            for each_suffix in suffix:
                if word.endswith(each_suffix):
                    new_word = "X-" + each_suffix
                    break
