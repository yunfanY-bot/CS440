...
return []
"""
def get_dim(alien, window, granularity):
    to_return = []

    # get configs
    alien.set_alien_pos((0, 0))
    top_left_config = alien.get_config()
    alien.set_alien_pos((window[0], 0))
    top_right_config = alien.get_config()
    alien.set_alien_pos((0, window[1]))
    low_left_config = alien.get_config()
    alien.set_alien_pos((window[0], window[1]))
    low_right_config = alien.get_config()

    # convert
    to_return.append(configToIdx(top_right_config, [0, 0], granularity, alien)[0] - configToIdx(top_left_config, [0, 0], granularity, alien)[0] + 1)
    to_return.append(configToIdx(low_left_config, [0, 0], granularity, alien)[1] - configToIdx(top_left_config, [0, 0], granularity, alien)[1] + 1)
    return to_return
...
def transformToMaze(alien, goals, walls, window,granularity):
    ...
    """
    start_config = alien.get_config()
    start_pos = configToIdx(start_config, [0, 0], granularity, alien)
    start_shape = start_config[2]

    # initialize maze and build window
    dim = get_dim(alien, window, granularity)
    input_map = []
    ncol = dim[0]
    nrow = dim[1]
    for i in range(ncol):
        ...
        for j in range(nrow):
            input_map[i].append([])
            for k in range(3):
                if i == 0 or j == 0 or i == ncol - 1 or j == nrow - 1:
                    input_map[i][j].append('%')
                else:
                    input_map[i][j].append(' ')
    # fill goals and walls
    shape = ['Horizontal', 'Ball', 'Vertical']
    for i in range(ncol):
        for j in range(nrow):
            for k in range(3):
                cur_config = idxToConfig((i, j, k), (0, 0), granularity, alien)
                alien.set_alien_config(cur_config)
    ...
    # lable start
    input_map[start_pos[0]][start_pos[1]][shape.index(start_shape)] = 'P'


    to_return = Maze(input_map=tuple(input_map), alien=alien, granularity=granularity)
    return to_return
